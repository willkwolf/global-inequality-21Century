...
test('DOM #pv-toggle: Conmutación interactiva entre Valor Presente y Nominal', () => {
  ...
  assert.ok(s1Caption.textContent.includes('USD*'), 'En modo PV, el caption debe incluir USD*');
  assert.ok(s1Date.textContent.includes('Valor presente') || s1Date.textContent.includes('Present value'));
  ...
  assert.ok(s1Caption.textContent.includes('USD '));
  ...
  assert.equal(pvBtn.getAttribute('aria-checked'), 'true');
  assert.ok(s1Caption.textContent.includes('USD*'));
  assert.ok(s1Date.textContent.includes('Valor presente'));
